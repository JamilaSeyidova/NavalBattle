...
public class BattleField extends GCompound {
	...
	private CellInfo[][] mapInfo = new CellInfo[MAP_LEN][MAP_LEN];
	

	public static int LENGTH = (int)(GameServer.APPLICATION_HEIGHT*COEF);
...
//		}
		...
		for (int i = row; i < row + s.getLen_h(); i++) {
			for (int j = col; j < col + s.getLen_v(); j++) {
				...
				map[i][j].setCellShipType(s.getShipType());
				mapInfo[i][j].setShipType(s.getShipType());
			}
		}
	...
	public CellInfo[][] getMapInfo() {
		return mapInfo;
	}

	public void set_bf(CellInfo[][] enemyMapInfo) {
		for (int row = 0; row < map.length; row++) {
			for (int col = 0; col < map.length; col++) {
				if(enemyMapInfo[row][col].getShipType() == ShipType.NONE) continue; 
				map[row][col].setCellStatus(CellStatus.SHIP_ORIGINAL);
				map[row][col].setCellShipType(enemyMapInfo[row][col].getShipType());
			}
		}
		
		mapInfo = enemyMapInfo;
	}

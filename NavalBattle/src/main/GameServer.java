...
public class GameServer extends GraphicsProgram {
...
//			enemy_bf.sendCellToFront(attackedCell);
		...
		ServerServices.sendAttackInfo(attackedCell.getLocation().toPoint());
	}
...
//			if(!isOnlineMode) return;
			...
			ServerServices.sendMessage(curr_actor_ready);
		}
...
//			
//			actor2_ready = true;
//			btn_start.setEnabled(actor1_ready);
//		}
...
//				gobj.setLocation(ship_coord);
		...
		if (e.getActionCommand().equals("Start")) {
			...
			ServerServices.sendOwnMap(curr_actor.getGameMap().getOwn_bf().getMapInfo());
			
			CellInfo[][] enemyMap = ServerServices.receiveEnemyMap();
			enemy_actor = new Actor(ACTOR2_NAME, new GameMap(ACTOR2_NAME));
...
//		curr_actor.getGameMap().setVisible(true);
	...
	public void run() {
		...
		while(true) {
			...
			Point attackedPoint = ServerServices.waitForAttack();
	...
	boolean curr_actor_ready, actor2_ready;
